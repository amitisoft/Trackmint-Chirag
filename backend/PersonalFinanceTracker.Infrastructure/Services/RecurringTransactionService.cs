    ...
    IAuditService auditService) : IRecurringTransactionService
{
    public async Task<IReadOnlyCollection<RecurringTransactionResponse>> GetAllAsync(CancellationToken cancellationToken)
    {
        ...
        var items = await Query(userId).ToListAsync(cancellationToken);
    ...
    {
        var userId = currentUserService.GetUserId();
        var item = await dbContext.RecurringTransactions.SingleOrDefaultAsync(x => x.Id == id && x.UserId == userId, cancellationToken)
    ...
    private IQueryable<RecurringTransaction> Query(Guid userId) =>
